...
class HrTime {
  ...
  get text() {
    ...
    return (
      t[0].toString(10) + "." + ("0".repeat(9 - ns.length) + ns).slice(0, 6)
    );
  }
...
function startAdapter(options) {
  ...
  options = Object.assign(options, {
    ...
    async unload(callback) {
      try {
        await Promise.resolve(_onUnload ? _onUnload(null) : null).catch((e) =>
          MyAdapter.Wf(e)
        );
    ...
    objectChange(id, obj) {
      ...
      if (typeof _objChange == "function")
        setImmediate(() => _objChange(id, obj));
      if (obj) {
    ...
    stateChange(id, state) {
      ...
      if (
        _stateChange &&
        (!state || state.from !== "system.adapter." + MyAdapter.ains)
      )
        setImmediate(() =>
...
class MyAdapter {
  ...
  static getObjects(name) {
    ...
    if (name) {
      ...
      opt.endkey =
        (name.startsWith("system.") ? "" : this.ain) + name + "\u9999";
    }
  ...
  static async initAdapter() {
    try {
      ...
      this.removeState = async (id, opt) => {
        ...
        await adapter
          .delObjectAsync((delete states[id], id), opt)
          .catch(this.nop);
      };
      ...
      res = await adapter
        .getForeignObjectAsync("system.adapter." + this.ains)
        .catch(() => null);
      if (res) {
        ...
        if (
          adapter.config.loglevel === "debug" ||
          adapter.config.loglevel === "silly"
        )
          this.debug = true;
  ...
  static init2() {
    ...
    this.removeState = (id, opt) =>
      adapter
        .delStateAsync(id, opt)
        .then(() => this.delObject((delete states[id], id), opt));
...
*/
  ...
  static D(str, val) {
    ...
    return (
      inDebug ? slog("info", `debug: ${str}`) : slog("debug", str),
    ...
    const nv = [],
      f =
        delay > 0
          ? (k) =>
              (p = p.then(() =>
                promfn(k).then((res) => this.wait(delay, nv.push(res)))
              ))
          : (k) => (p = p.then(() => promfn(k)));
...
*/
  ...
  static async changeState(id, value, options) {
    ...
    if (value === undefined) {
      this.Wf(
        "You tried to set state '%s' to 'undefined' with %j!",
        id,
        options
      );
      return null;
    ...
    await adapter
      ...
      .catch(
        (e) => (this.Wf("Error %j is setState for %s with %j", e, id, stn), stn)
      );
    if (states[id]) {
      st = states[id];
      st.val = value;
  ...
  static getClass(obj) {
    ...
    const ret = Object.prototype.toString
      .call(obj)
      .match(/^\[object\s(.*)\]$/)[1];
