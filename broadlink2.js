...
A.objChange = async function (obj, val) { //	This is needed for name changes
  ...
  if (typeof obj === "string" && obj.indexOf(learnedName) > 0)
    try {
      ...
      if (nid === dev[4] || nid.startsWith(defaultName))
        // no need to rename!
        return null;
      if (
        !A.states[fnn]
          ? !oobj.native.code
            ? A.W(
                `Cannot rename to ${oobj.common.name} because it does not have a learned code: ${obj}`,
                true
              )
            : false
          : A.W(
              `Cannot rename to ${ncn} because the name is already used: ${obj}`,
              true
            )
      ) {
        oobj.common.name = dev[4];
        return A.setObject(obj, oobj).catch((e) =>
          A.W(`rename back err ${e} on ${A.O(oobj)}!`)
        );
      }
      ...
      if (nid !== dev[4]) {
        ...
        await A.removeState(A.I(`rename ${obj} to ${fnn}!`, obj)).catch(
          A.nothing
        );
        A.wait(100);
      }
...
function sendCode(device, value) {
  ...
  return device
    .sendVal(buffer)
    .then((x) => device.name + " sent " + value + ", " + x);
...
A.stateChange = async function (id, state) {
  ...
  async function startLearning(name, type) {
    ...
    if (thisDevice.learning) {
      const err = A.W(
        `Device ${name} is still in learning mode and cannot start it again!`
      );
      throw new Error(err);
    }
    ...
    const l =
      type === learnRf && thisDevice.learnRf
        ? await thisDevice.learnRf(learnMsg)
        : await thisDevice.learn(learnMsg);
    ...
    if (l.data)
      try {
        ...
        for (let i of res.rows)
          if (i.doc.native.code === hex)
            // ? i.doc.common.name
            return A.I(
              `Code alreadly learned from: ${thisDevice.name} with ${
                i.doc.common.name + ":" + i.doc._id
              }`
            );
        await A.makeState(
          {
  ...
  function checkT1(device, state, id) {
    ...
    for (let e of [
      ...
      if (id.endsWith(e)) return device.setAdvanced(e.slice(1), state.val);
    ...
    return A.reject(
      A.Wf(
        "checkT1 don't know how to set %s of %s to %O",
        id,
        device.name,
        state.val
      )
  ...
  if (state && !state.ack) {
    if (id.startsWith(A.ain)) id = id.slice(A.ain.length);
    let idx = id.split("."),
      id0 = idx[0],
      temp = {};
    ...
    if (id0 === scenesName) {
      ...
      if (obj && obj.native && obj.native.scene)
        await sendScene(obj.native.scene, state);
      else A.D(`Invalid command "${id}" in scenes`);
      return true;
    }
    if (id0 === statesName) {
      const obj = await A.getObject(id);
      if (obj && obj.native && obj.native.state)
        await sendState(obj.native.state, state.val);
      else A.D(`Invalid command "${id}" in states`);
      return true;
    }
    let device = scanList[id0];
    if (!device)
      return A.W(`stateChange error no device found: ${id0} ${A.O(state)}`);
    switch (device.typ) {
      case "SP":
      ...
      case "RM":
        if (id.endsWith(sendName))
          return state.val.startsWith(codeName)
            ? sendCode(device, state.val)
      ...
      case "T1":
        ...
        if (temp && temp.here && device.update) await device.update(temp);
        break;
      case "LB":
      ...
      default:
        return A.W(
          `stateChange error invalid id type: ${id}=${id0} ${A.O(state)}`
        );
    }
  }
};
...
async function deviceScan() {
  ...
  if (brlink.scanning)
    throw new Error(
      A.W(
        `Scan operation in progress, no new scan can be started until it finished!`
      )
    );
  try {
    await A.makeState(
...
async function sendScene(scene, st) {
  ...
  const sn = s.map((ss) =>
    A.trim(ss) === parseInt(ss).toString() ? parseInt(ss) : A.trim(ss)
  );
  let ff = true;
  for (const jj of sn) {
    let i = jj;
    if (ff) await A.wait(50);
    ff = true;
    if (typeof i === "number") {
      await A.wait(i);
      continue;
    ...
    if (mm) {
      await A.repeat(mm[1], () =>
        sendScene([mm[2]], st).then(() => A.wait(mm[3] ? mm[3] : 100))
      );
      continue;
    }
    if (i.split("=").length === 2) {
    ...
    if (f.length === 1) i = f[0]._id;
    else if (f.length > 1)
      A.W(`Multiple states with name '${i}: ${f.map((x) => x._id)}`);
    if (i.startsWith(A.ain)) i = i.slice(A.ain.length);
...
A.messages = async (msg) => {
  ...
  let id = msg.message.startsWith(A.ain)
    ? msg.message.trim()
    : A.ain + msg.message.trim();
  let ids, idx, code;
  ...
  switch (msg.command) {
      ...
      return A.getObject(id)
        .then(
          (obj) =>
            ...
            (obj.common.role === "switch" && msg.command.startsWith("switch"))
              ? A.stateChange(id, st)
              : Promise.reject(
                  A.W(`Wrong id or message ${A.O(msg)} id = ${A.O(obj)}`)
                ),
    ...
    case "switch":
      ...
      if (idx.length !== 2 && !idx.startsWith("SP:"))
        return Promise.reject(
          A.D(`Invalid message to "switch" ${msg.message}" to ${idx}`)
        );
    ...
    default:
      return Promise.reject(
        A.D(
          `Invalid command "${msg.command}" received with message ${A.O(msg)}`
        )
      );
  }
};
  ...
  for (const item of Object.entries(scanList)) {
    ...
    if (!x.here) {
      ...
      if (
        ...
        A.W(
          `Device ${device.name} not reachable, switched it off and will search for it every while.`
        );
        await A.makeState(
          device.name + reachName,
          (device.unreach = true),
          true
        );
      }
...
function setState(name) {
  ...
  for (let k in states) {
    const st = states[k];
    let where = A.includes(st.on, name)
      ? st.on
      : A.includes(st.off, name)
      ? st.off
      : null;
    if (where) {
      str = {
  ...
  if (str.index < 0)
    // was found in off so switch state off!
...
async function sendState(state, val) {
  ...
  if (A.T(val, true) && state.off) send = val ? state.on[0] : state.off[0];
  ...
  try {
    ...
    if (sobj.length > 1)
      A.W(
        `sendState error: multiple commands for name ${send}, ill use only first instance: ${sobj[0]._id}!`
      );
    if (!sobj.length)
      return A.W(
        `sendState error: Could not findcommand '${send}', to send for : ${state.name}!`
      );
...
async function updateValues(device, val, values) {
  ...
  for (const item of values) {
    ...
    if (v !== undefined) {
      if (typeof i.fun === "function") await i.fun(device, i, v);
      else await A.makeState(i, v, true);
    }
  }
}
...
function findName(name) {
  for (let i of rename) if (i[0] === name) return i[1];
  return null;
}
      ...
      device.update = async (val) => {
        ...
        if (val.state !== undefined && device.oval !== val.state) {
          if (device.oval !== undefined)
            A.I(
              `Switch ${device.host.iname} changed to ${val.state} most probably manually!`
            );
        ...
        await updateValues(device, val, [
          ...
          A.Wf("Update device %s Error: %O", device.host.name, e)
        );
      };
      break;
    case "S1":
    ...
    case "A1":
      device.update = (val) =>
        updateValues(device, val, [
          ...
          A.Wf("Update device %s Error: %O", device.host.iname, e)
        );
...
async function main() {
  ...
  rename = rename.map((x) => [
    x[0],
    x[1].replace(A.adapter.FORBIDDEN_CHARS, "_"),
  ]);
  if (A.C.additional) {
    if (!Array.isArray(A.C.additional)) {
      A.C.additional =
        typeof A.C.additional == "string"
          ? A.C.additional.split(",").map((i) => i.trim().toLowerCase())
          : [];
    }
    if (A.C.additional.length == 1 && !A.C.additional[0]) A.C.additional = [];
  } else A.C.additional = [];
  ...
  for (let i of A.ownKeys(A.objects))
    if (i.startsWith(A.ain)) adapterObjects.push(A.objects[i]);
  const hosts = adapterObjects
    .filter((i) => i._id.split(".").length === 3 && i.native && i.native.host)
    .map((i) => i.native);
  ...
  brlink.on("deviceReady", async (device) => {
    ...
    if (oldDevv && device.host.name != oldDevv.host.name) {
      ...
      A.D(
        `Device '${device.host.oname}' renamed to '${device.host.name} because it was found in device list.`
      );
    }
    ...
    if (x && x != device.host.name)
      A.I(
        `Found new device (${device.host.name}) with mac ${device.host.mac} and gave it previous known name ${x}`
      );
  ...
  for (const obj of hosts) {
    ...
    if (macObjects[omac]) {
      ...
      if (n1 != n2)
        A.W(
          `same broadlink mac in two different devices: '${omac}' in '${n1}' and '${n2}', will use first!`
        );
  ...
  try {
    ...
    for (const state of A.C.switches) {
      ...
      if (!off) {
        ...
        if (mult) option.states = null;
        else option.states = on.map((s, i) => `${i}:${s.trim()}`).join(";");
      }
      option.native = {
        state: {
          id: option.id,
    ...
    for (const item of Object.entries(scanList)) {
      ...
      if (oname != iname) {
        if (oobj && iobj) {
          A.Wf(
            `Got item which had original name of %s and new name of %s,\n please delete new one to get old renamed or old one if you don't need it's items anymore.`,
            oname,
            iname
          );
    ...
    for (const dev of A.obToArray(A.objects).filter(
      (x) => x._id.startsWith(A.ain) && x.native && x.native.host
    )) {
      ...
      if (
        ...
        !findName(id) &&
        !scanList[id] &&
        dev._id === A.ain + dev.common.name &&
        dev.common.name.indexOf(".") < 0
      ) {
        ...
        if (brlink.getDev(dev.native.host.mac)) {
          ...
          A.Wf(
            "seems that device %s got renamed to %s! You may delete old device and change your scripts!",
            id,
            device.name
          );
        } else {
          A.W(
            `device ${id} not found, please rescan later again or delete it! It was: ${A.obToArray(
              device.host
            )}`
          );
          scanList[id] = device;
    ...
    for (const k of Object.keys(macObjects)) {
      ...
      if (!f) {
        ...
        if (notFound.indexOf(m.host.name) < 0) notFound.push(m.host.name);
      }
    }
    await doPoll();
    await A.makeState(
    ...
    A.I(
      `${A.ains} started and found ${
        didFind.length
      } devices named ${didFind.join(", ")}`
    );
    if (notFound.length > 0)
      A.I(`${notFound.length} were not found: ${notFound.join(", ")}`);
