...
function translateAll(selector, lang, dictionary) {
	...
	$selector.find(".translate").each(function (idx) {
		let text = $(this).attr("data-lang");
		if (!text) {
			text = $(this).html();
			$(this).attr("data-lang", text);
		}

		const transText = translateWord(text, lang, dictionary);
		if (transText) {
			$(this).html(transText);
		}
	});
	...
	$selector.find(".translateV").each(function (idx) {
		let text = $(this).attr("data-lang-value");
		if (!text) {
	...
	$selector.find(".translateT").each(function (idx) {
		...
		let text = $(this).attr("data-lang-title");
		if (!text) {
	...
	$selector.find(".translateP").each(function (idx) {
		...
		let text = $(this).attr("data-lang-placeholder");
		if (!text) {
...
const _ = function (text, arg1, arg2, arg3) {
