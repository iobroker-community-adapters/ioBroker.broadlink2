...
const EventEmitter = require("events"),
  ...
  A = require("./fjadapter");
const { setForeignObject } = require("./fjadapter");
  ...
  constructor(add, aif) {
    ...
    for (let k of add) {
      if (Array.isArray(k) && k.length === 2) {
        ...
        let dt = Number(k[0]);
        for (const c of Object.keys(this._devlist)) if (c[dt]) delete c[dt];
  ...
  parsePublic(msg, rinfo) {
    ...
    if ((!self._devices[mac] || self._devices[mac].dummy) && host.devtype) {
      ...
      dev.once("deviceReady", function () {
        return (
          A.c2p(dns.reverse)(dev.host.address)
            .catch(() => dev.host.name)
      ...
      A.retry(3, dev.auth.bind(dev), 300).catch((err) =>
