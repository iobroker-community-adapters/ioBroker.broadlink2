...
Broadlink.prototype.genDevice = function (devtype, host, mac) {
	...
	for (let typ of [sp1, sp2, rm, a1, mp1])
...
Broadlink.prototype.discover = function (what) {
	var self = this;
	...
	if (what) {
		setTimeout(() => {try {cs.close();} catch(e) {}},2000);
		if (what.mac)
			self.devices[what.mac] = undefined;
	...
	cs.on('listening', function () {
		...
		cs.sendto(packet, 0, packet.length, 80, self.ip || (what && what.address) || '255.255.255.255');
	});
...
function Device(host, mac, devtype) {
	var self = this;
