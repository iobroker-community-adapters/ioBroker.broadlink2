...
vis.binds.broadlink = {
    ...
    showVersion: function () {
        if (vis.binds.broadlink.version) {
            ...
            vis.binds.broadlink.version = null;
        }
    },
	createWidget: function (widgetID, view, data, style) {
        ...
        if (!$div.length) {
            return setTimeout(function () {
                vis.binds.broadlink.createWidget(widgetID, view, data, style);
            }, 100);
        }
        ...
        if (data.oid) {
            vis.states.bind(data.oid + '.val', function (e, newVal, oldVal) {
                $div.find('.broadlink-value').html(newVal);
            });
        }
    }
};
	
vis.binds.broadlink.showVersion();
